...
class Instance(api.Instance):
    def __init__(self, model_path: str, model_name: str, device: api.Device):
    ...
    def inference(self, *, data: dict[str, Any], report: dict[str, Any], **kwargs: Any) -> dict[str, Any]:
        ...
        for the_landmarks in face_landmarks:
            ...
            face_expression.append({k: v for k, v in zip(self.__class_map["expression"], cls_pred, strict=False)})
            face_action_units.append({k: v for k, v in zip(self.__class_map["action_units"], au_pred, strict=False)})
